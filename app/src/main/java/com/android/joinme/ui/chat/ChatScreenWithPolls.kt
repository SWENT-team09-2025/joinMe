...
/** Test tags for ChatScreenWithPolls. */
object ChatWithPollsTestTags {
  const val POLLS_SECTION = "pollsSection"
  const val POLL_CREATION_SHEET = "pollCreationSheet"
}
        ...
        messages
            .map { message ->
              if (message.type == MessageType.POLL) {
                ...
                val poll = pollsMap[message.content]
                if (poll != null) {
                  ChatTimelineItem.PollItem(poll)
                } else {
                  // Poll not found (might be deleted), skip this message
                  null
                }
              } else {
