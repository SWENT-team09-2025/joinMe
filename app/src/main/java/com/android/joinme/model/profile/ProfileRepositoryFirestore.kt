...
class ProfileRepositoryFirestore(db: FirebaseFirestore, private val storage: FirebaseStorage) :
    ...
    /**
     * Retrieves a user profile by UID from Firestore. Throws [NoSuchElementException] if the
    ...
    /**
     * Deletes the profile photo for the given user UID from Firebase Storage and clears the
    ...
    override suspend fun deleteProfilePhoto(uid: String) {
        try {
            ...
            try {
                ...
                Log.w(
                    TAG,
                    "Photo file not found in Storage, continuing to clear Firestore field",
                    e
                )
            }
